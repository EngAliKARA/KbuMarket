...
class Home extends CI_Controller {
    ...
    public function index()
    {
        ...
        if ($this->session->userdata('CustomerID'))
            ...
            $data["SC"]=0;
        $data["Sayfalar"] = $this->Front_Model->GetSayfaList();
        $data["Settings"] = $this->Front_Model->GetSettings();
    ...
    public function Category($Id){
        ...
        $data["Sayfalar"] = $this->Front_Model->GetSayfaList();
        $Category = $this->Front_Model->GetKategoriDetay($Id);
        $Settings = $this->Front_Model->GetSettings();
        $Settings->Adi = $Category->KategoriAdi;
    ...
    public function Product($Id){
        ...
        $data["Yorumlar"]=$this->Front_Model->GetYorumList($Urun->ID);
        $data["Sayfalar"] = $this->Front_Model->GetSayfaList();
        $Settings = $this->Front_Model->GetSettings();
        $Settings->Adi = $Urun->UrunAdi;
        $Settings->Keywords = $Urun->Keywords;
        $data["Settings"] = $Settings;
    ...
    public function CustomerKaydet(){
        ...
        $GuvenlikKodu = $this->security->xss_clean($this->input->post("Captcha"));
        ...
        if ($CaptchaKodu == $GuvenlikKodu) {
            ...
            if ($Customer) {
                ...
                if ($Sonuc) {
                    ...
                    if (!$Send) {
                        $this->session->set_flashdata("Message", "Mail Gönderiminde Hata Oluştu Hata Kodu,".$this->email->print_debugger());
                        $this->session->set_flashdata("Kod", "3");
                        redirect($this->agent->referrer());
                    } else {
    ...
    public function Yorumlar(){
        if ($this->session->userdata('CustomerID')){
            ...
            if ($this->session->userdata('CustomerID'))
                ...
                $data["SC"] = 0;
            $data["Yorumlar"] = $this->Front_Model->GetCustomerYorumList($CustomerID);
            $data["Settings"] = $this->Front_Model->GetSettings();
            $this->load->view('Customer/Yorumlarim', $data);
        } else{
            $this->session->set_flashdata("Message", "Yorumlarınızı Görüntüleyebilmek için Giriş Yapmalısınız!!!");
            $this->session->set_flashdata("Kod", "1");
            redirect($this->agent->referrer());
        }
    }
    public function YorumSil($Id){
        $Sonuc = $this->Front_Model->SetDeleteYorum($Id);
        if ($Sonuc==1){
    ...
    public function YorumDetay($Id){
        if ($this->session->userdata('CustomerID')){
            ...
            if ($this->session->userdata('CustomerID'))
                ...
                $data["SC"] = 0;
            $data["Yorum"] = $this->Front_Model->GetYorumDetay($Id);
            $data["Settings"] = $this->Front_Model->GetSettings();
            $this->load->view('Customer/YorumDetay', $data);
        } else{
            $this->session->set_flashdata("Message", "Yorumlarınızı Görüntüleyebilmek için Giriş Yapmalısınız!!!");
            $this->session->set_flashdata("Kod", "1");
            redirect($this->agent->referrer());
    ...
    public function MesajKaydet(){
        if ($this->session->userdata('CustomerID')) {
            ...
            $Sonuc = $this->Front_Model->MesajKaydet($Mesaj);
            if ($Sonuc) {
                $this->session->set_flashdata("Message", "Mesajınız Başarıyla İletilmiştir. ".
                    "İletişim Mesajları bölümünden Takip edebilirsiniz");
                $this->session->set_flashdata("Kod", "0");
                redirect($this->agent->referrer());
            }
            else {
                $this->session->set_flashdata("Message", "Mesajınız İletilemedi, Lütfen Yetkililer ile İritabata Geçiniz...");
                $this->session->set_flashdata("Kod", "3");
                redirect($this->agent->referrer());
            }
        }
        else {
            $this->session->set_flashdata("Message", "Bize Yazabilmek İçin Giriş Yapmalısınız!!!");
            $this->session->set_flashdata("Kod", "3");
            redirect($this->agent->referrer());
        }
    }
    public function Mesajlar(){
        if ($this->session->userdata('CustomerID')){
            ...
            if ($this->session->userdata('CustomerID'))
                ...
                $data["SC"] = 0;
            $data["Mesajlar"] = $this->Front_Model->GetMesajList($CustomerID);
            $data["Settings"] = $this->Front_Model->GetSettings();
            $this->load->view('Customer/Mesajlarim', $data);
        } else{
            $this->session->set_flashdata("Message", "Mesajlarınızı Görüntüleyebilmek için Giriş Yapmalısınız!!!");
            $this->session->set_flashdata("Kod", "1");
            redirect($this->agent->referrer());
        }
    }
    public function MesajDetay($Id){
        if ($this->session->userdata('CustomerID')){
            ...
            if ($this->session->userdata('CustomerID'))
                ...
                $data["SC"] = 0;
            $data["Mesaj"] = $this->Front_Model->GetMesajDetay($Id);
            $data["Settings"] = $this->Front_Model->GetSettings();
            $this->load->view('Customer/MesajDetay', $data);
        } else{
            $this->session->set_flashdata("Message", "Yorumlarınızı Görüntüleyebilmek için Giriş Yapmalısınız!!!");
            $this->session->set_flashdata("Kod", "1");
            redirect($this->agent->referrer());
    ...
    public function MesajSil($Id){
        $Sonuc = $this->Front_Model->SetDeleteMesaj($Id);
        if ($Sonuc==1){
    ...
    public function MesajYanitSil($Id){
        $Sonuc = $this->Front_Model->SetDeleteYanit($Id);
        if ($Sonuc==1){
    ...
    public function Sayfa($Id) {
        ...
        if ($this->session->userdata('CustomerID')) {
            ...
            $data["SC"] = 0;
        $data["Sayfalar"] = $this->Front_Model->GetSayfaList();
        $data["Sayfa"] = $this->Front_Model->GetSayfaDetay($Id);
        $data["Settings"] = $this->Front_Model->GetSettings();
