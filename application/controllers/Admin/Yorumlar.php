...
class Yorumlar extends  CI_Controller
{
    ...
    public function Index() {
        $data["Yorumlar"]=$this->Main_Model->GetYorumList(0);
        $this->load->view("Admin/Yorumlar/Yorum_list",$data);
    }
    public function Duzenle($Id) {
        $data["Yorum"] = $this->Main_Model->GetYorumDetay($Id);
    ...
    public function Guncelle(){
        ...
        $Sonuc = $this->Main_Model->YorumGuncelle($YorumID,$YorumNeden,$Durum);
        if ($Sonuc==1){
    ...
    public function Sil($Id){
        $Sonuc = $this->Main_Model->SetDeleteYorum($Id);
        if ($Sonuc==1){
